...
# Dynamic pages, such as those requiring JavaScript loading, invoke the Selenium package from the video interface to simulate interface clicking, loading, and waiting for the JS loading to complete
# Selenium package is easier, but it is slower.
...
class Model():
    ...
    def process_html(self, html):
        ...
        name = soup.find(class_='TitleBlock__TitleContainer-sc-1nlhx7j-1 jxsVNt').h1.get_text()
        # 去掉html的一些/x20等空白符
        ...
        # 海报的URL
        poster_url = soup.find(class_='ipc-media ipc-media--poster ipc-image-media-ratio--poster ipc-media--baseAlt ipc-media--poster-l ipc-poster__poster-image ipc-media__img').img['src']
        print(poster_url)
        ...
        # 视频链接
        try:
            #classname = "ipc-lockup-overlay Slatestyles__SlateOverlay-sc-1t1hgxj-2 fAkXoJ hero-media__slate-overlay ipc-lockup-overlay--hover ipc-focusable"
            classname = 'ipc-slate ipc-slate--baseAlt ipc-slate--dynamic-width Slatestyles__SlateContainer-sc-1t1hgxj-0 hZESQm undefined celwidget ipc-sub-grid-item ipc-sub-grid-item--span-4'
            vurl = soup.find(class_=classname).a['href']
            vurl = "https://www.imdb.com" + vurl
            print(vurl)
            self.save_videolist(self.cur_movie_id,vurl)
        except AttributeError as e:
            print("can not get video url")

        classname = "ipc-metadata-list ipc-metadata-list--dividers-all Storyline__StorylineMetaDataList-sc-1b58ttw-1 esngIX ipc-metadata-list--base"
        # 基本信息和详细发布时间 Animation, Adventure, Comedy | 21 March 1996 (Germany)
        for tag in soup.find(class_=classname).find_all('a'):
            info.append(tag.get_text().strip())
        # 简介
        intro = soup.find(class_='ipc-html-content ipc-html-content--base').get_text().strip()
    ...
    def save_videolist(self,index,url):
        with open('videopage.txt','a+') as fb:
            fb.write(index)
            fb.write('\n')
            fb.write(url)
            fb.write('\n')
